...
- **2025-11-12**: 🎛️ **UI·spacing/히스토리 정리**
  - `--space-xs`~`--space-2xl` 토큰을 정의하고 번역/히스토리/검색/설정 탭 전체의 여백을 일관된 값으로 맞춤
  - 히스토리 삭제 버튼과 텍스트 번역 탭의 아이콘/정렬/간격을 통일하고, 전체 삭제는 클릭-확인-자동복귀 흐름으로 변경
  - 번역 결과 텍스트에 `white-space: pre-wrap`을 적용해 줄바꿈을 유지하고, 히스토리 섹션 상단에 32px 여백을 둬 구분을 명확히 함
