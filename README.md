...
1. **새 버전 내려받기**
   - 이 저장소에서 최신 ZIP을 다시 다운로드하고, 기존과 다른 위치에 압축을 풉니다.
   - 기존 폴더를 덮어쓰지 말고 새 폴더로 받아 두면 롤백이 필요할 때 쉽게 되돌릴 수 있어요.
2. **확장 프로그램 화면 열기**
   - Chrome 주소창에 `chrome://extensions/` (웨일: `whale://extensions/`, Comet: `comet://extensions/`)을 입력해 확장 프로그램 관리 화면으로 이동합니다.
3. **기존 버전 교체하기**
   - 목록에서 "무조건 한글로 번역해드림" 항목을 찾고 **삭제** 버튼 대신 **세부 정보 → 폴더 위치 열기**를 눌러 현재 경로를 확인합니다.
   - Finder/탐색기에서 해당 폴더를 새로 받은 폴더 내용으로 전체 교체하거나, Chrome 화면에서 **"압축 해제된 확장 프로그램을 로드합니다"** 버튼으로 새 폴더를 다시 선택하세요.
4. **새 버전 적용 확인하기**
   - 확장 프로그램 카드에서 **"다시 로드"** 버튼을 누르면 브라우저에 로드된 코드가 갱신돼요.
   - 사이드패널 하단 푸터의 최종 수정 날짜가 최신 버전과 일치하는지 확인해 주세요.
